...
def clean_amount(val):
    try:
        num = float(val.replace(",", ""))
        if num <= 0 or num > 500000:  # ตรวจสอบความสมเหตุสมผล
            return ""
        return f"{num:.2f}"
    except:
        return ""
...
# Streamlit App
# ================================
st.set_page_config(page_title="OCR + Docling Invoice Tool", layout="wide")
st.title("📄 OCR + Docling Invoice Extractor")
st.write("อัปโหลด PDF / รูปภาพ → ดึงข้อมูลด้วย Docling หรือ OCR → แก้ไขใต้ภาพ → ดาวน์โหลด Excel")
...
if uploaded_file:
    ...
    if not st.session_state.results:
        if uploaded_file.type == "application/pdf":
            ...
            with st.spinner("กำลังพยายามอ่านด้วย Docling..."):
                try:
                    # ใช้ Docling
                    text = run_docling(pdf_bytes)
                    # แปลง PDF เป็นภาพไว้ preview
                    ...
                    st.warning(f"Docling ล้มเหลว: {e} → จะใช้ OCR แทน")
                    pages = convert_from_bytes(pdf_bytes, dpi=400)
                    st.session_state.results = run_ocr(pages)
        else:
            img = Image.open(uploaded_file).convert("RGB")
            st.session_state.results = run_ocr([img])
    ...
    for row in st.session_state.results:
        st.markdown(f"### หน้า {row['page_number']}")
        img_base64 = pil_to_base64(row["image"])
        st.markdown(f'<img src="data:image/png;base64,{img_base64}" class="center-img"/>',
                    unsafe_allow_html=True)
        ...
        with col1:
            date_val = st.text_input(
                f"วันที่ หน้า {row['page_number']}",
                value=row["date"],
                key=f"date_{row['page_number']}"
            )
        with col2:
            inv_val = st.text_input(
                f"เลขที่บิล หน้า {row['page_number']}",
                value=row["invoice_number"],
                key=f"inv_{row['page_number']}"
            )
        with col3:
            amt_val = st.text_input(
                f"ยอดก่อน VAT หน้า {row['page_number']}",
                value=row["amount"],
                key=f"amt_{row['page_number']}"
            )
